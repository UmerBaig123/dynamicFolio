...
def home_page(request):
    ...
    return render(request, 'home.html', {"userdata": userdata_obj, "repos": repos,"publications":publications})
def repository(request):
    ...
    if selected_repos.objects.all().count() == 0:
        return render(request, 'repository.html', {"repos":"none","github_username":github_username,"description_text":description_text})
    repos = [repo.repo_id for repo in selected_repos.objects.all()]
    repos = ",".join(repos)
    return render(request, 'repository.html', {"repos":repos,"github_username":github_username,"description_text":description_text})
def publications(request):
    ...
    publications = publication.objects.all()
    return render(request, 'publications.html', {"publications":publications,"description_text":page_desc})
def teachings(request):
    ...
    return render(request, 'teachings.html', {"description": description_text,"courses":myCourses})
def resume(request):
    education_exp = []
    professional_exp = []
    academic_exp = []   
